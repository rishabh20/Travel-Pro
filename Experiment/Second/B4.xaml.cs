...
namespace Experiment
{
    ...
    /// </summary>
    public sealed partial class B4 : Page
    {
        ...
        private async void Info_Tapped(object sender, TappedRoutedEventArgs e)
        {
            if (hello == 0)
            {
                try
                {
                    var url = await YouTube.GetVideoUriAsync("VuS6YpyT06w", YouTubeQuality.Quality360P);
                    Info.Source = url.Uri;
                    hello = 1;
                }
                catch
